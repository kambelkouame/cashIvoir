...
public class RegisterActivity extends AppCompatActivity {
    ...
    EditText username, email,tel, password, confirmPassword; //Declaration EditTexts
    TextInputLayout txtUsername, txtEmail,numtel, txtPassword, txtConfirmPassword; //Declaration TextInputLayout
            ...
            public void onClick(View view) {
                if (validate()) {
                    ...
                    if (cursor.getCount()>0) {
                        ...
                        Toast.makeText(getApplicationContext(), "utilisateur existe deja",
                                Toast.LENGTH_LONG).show();
                    }else{

                        SQLiteDatabase query = dataHelper.getWritableDatabase();
                        query.execSQL("insert into users(username, email,tel, password) values('" +
                                UserName + "','" +
                                Email + "','" +
                                Tel + "','" +
                                Password + "')");
                        Toast.makeText(getApplicationContext(), "utilisateur crée avec success!! veiller vous identifier",
                                Toast.LENGTH_LONG).show();
